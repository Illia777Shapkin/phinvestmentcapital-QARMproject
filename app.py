...
def run_backtest_cached(config_dict, data):
    cfg = PortfolioConfig(**config_dict)
    return run_backtest(cfg, data)


@st.cache_data(show_spinner=False)
def run_today_optimization_cached(config_dict, data):
    cfg = PortfolioConfig(**config_dict)
    return run_today_optimization(cfg, data)
    ...
    if run_backtest_clicked:
        ...
        try:
            with st.spinner("Running backtest..."):
                perf, summary_df, debug_weights_df = run_backtest_cached(config_dict, data)
        except ValueError:
            st.error(
    ...
    if run_today_clicked:
        ...
        try:
            with st.spinner("Computing current optimal allocation..."):
                today_res = run_today_optimization_cached(config_dict, data)
        except ValueError:
            st.error(
        ...
        st.session_state.ai_messages = [
            {
                ...
                "content": (
                    ...
                    • **Meet the Team**
                      ...
                      - Team members: Illia Shapkin (CEO & Quant & Portfolio Strategy) , Rean Morinaj  (CIO & Quantitative Research), Malek Trimeche (Client Relations), Petrit Gashi (CFO - Finance & Operations)
